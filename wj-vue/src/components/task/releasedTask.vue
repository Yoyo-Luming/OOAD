<template>
      ...
      <el-main style="height: 100%;padding: 0;">
        <el-container class="mid-content">
          <el-tabs type="border-card" v-model="activePane">
            <!--          take-pane-->
            <el-tab-pane label="待接受" class="whole-pane" name="first">
              <div v-if="waitGetTaskList.length">
                <el-container class="whole-order" v-for="(item, index) in waitGetTaskList" :key="index">
            ...
            <!--          pay-pane-->
            <el-tab-pane label="待领货" class="whole-pane" name="second">
            ...
            <!--          send-pane-->
            <el-tab-pane label="待送达" class="whole-pane" name="third">
            ...
            <!--          receive-pane-->
            <el-tab-pane label="待收货" class="whole-pane" name="fourth">
            ...
            <!--          evaluate-pane-->
            <el-tab-pane label="待评价" class="whole-pane" name="fifth">
            ...
            <!--          finished-pane-->
            <el-tab-pane label="已完成" class="whole-pane" name="sixth">
  ...
  <!--  <el-container>-->
  <!--    <el-tabs>-->
  <!--      <el-tab-pane label="待接受的任务">-->
  <!--        <task-order-box v-for="(item, index) of waitGetTaskList" :key="index+Math.random()" :taskName="item.taskName" :price="item.price" :startRegion="item.startRegion" :endRegion="item.endRegion" :detail="item.detail"></task-order-box>-->
  <!--      </el-tab-pane>-->
...
export default {
  ...
  mounted () {
    ...
    // 待领货的任务
    this.$axios.post('task/task_wait_receive_object_list_up/').then(response => {
      console.log(response.data)
      let len = response.data.return_transaction.length
      let list = response.data.return_transaction
      console.log(len)
    ...
    // 待送达的任务
    this.$axios.post('task/task_wait_send_to_place_list_up/').then(response => {
      console.log(response.data)
      let len = response.data.return_transaction.length
      let list = response.data.return_transaction
      for (let i = 0; i < len; ++i) {
    ...
    // 待收货确认的任务
    this.$axios.post('task/task_wait_confirm_receive_list_up/').then(response => {
      console.log(response.data)
      let len = response.data.return_transaction.length
      let list = response.data.return_transaction
      for (let i = 0; i < len; ++i) {
    ...
    // 待评价的任务
    this.$axios.post('task/task_wait_confirm_comment_list_up/').then(response => {
      console.log(response.data)
      let len = response.data.return_transaction.length
      let list = response.data.return_transaction
      for (let i = 0; i < len; ++i) {
    ...
    // 已完成的任务
    this.$axios.post('task/task_wait_confirm_success_list_up/').then(response => {
      console.log(response.data)
      let len = response.data.return_transaction.length
      let list = response.data.return_transaction
      for (let i = 0; i < len; ++i) {
